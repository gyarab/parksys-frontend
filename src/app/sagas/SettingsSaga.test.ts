...
describe("SettingsSaga", () => {
  describe("fetchTranslations", () => {
    it("gets translations and sets fulfilled", () => {
      expect.assertions(1);
      const dispatched = [];
      (dummyApi as any).getTranslations.mockResolvedValue({
        ...
        .then(() => {
          expect(dispatched).toEqual([
            { payload: null, type: "SETTINGS/SET_LANGUAGE_PENDING" },
            {
              ...
              type: "SETTINGS/SET_LANGUAGE_FULFILLED"
            }
          ]);
        });
    });
        ...
        .then(() => {
          expect(dispatched).toEqual([
            { payload: null, type: "SETTINGS/SET_LANGUAGE_PENDING" },
            {
              message: "Error",
              payload: null,
              type: "SETTINGS/SET_LANGUAGE_REJECTED"
            }
          ]);
        });
    });
    ...
    it("listens for setLanguage INVOKED and calls fetchTranslations", () => {
      ...
      expect(gen.next().value).toEqual(
        ReduxSagaEffects.takeLatest(
          getType(setLanguage.invoke),
          settingsSaga.fetchTranslations
        )
      );
    });
  });
